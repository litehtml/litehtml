...
void cairo::border::draw_border()
{
	...
	if(radius_bottom_x != 0_px && radius_bottom_y != 0_px)
	{
...
void cairo::border::draw_line(double line_offset, double top_line_offset, double bottom_line_offset)
{
	...
	{
		cairo_move_to(cr, left.value() + line_offset, top.value());
	}
	...
	if(radius_bottom_x != 0_px && radius_bottom_y != 0_px)
	{
		...
		add_path_arc(cr, left.value() + radius_bottom_x.value(), bottom.value() - radius_bottom_y.value(),
					 ...
					 radius_bottom_y.value() - line_offset + (line_offset - bottom_line_offset), start_angle, end_angle,
					 true);
	} else
	{
		cairo_line_to(cr, left.value() + line_offset, bottom.value());
	}
}
...
void cairo::border::draw_inset_outset(bool is_inset)
{
	...
	cairo_set_line_width(cr, border_width.value());
	cairo_stroke(cr);
}

void cairo::border::draw_double()
{
	if(border_width < 3_px)
	{
		draw_solid();
	} else
	{
		...
		double line_width = border_width.value() / 3.0;
		cairo_set_line_width(cr, line_width);
		// draw external line
		draw_line(line_width / 2.0, top_border_width.value() / 6.0, bottom_border_width.value() / 6.0);
		cairo_stroke(cr);
		// draw internal line
		draw_line(border_width.value() - line_width / 2.0, top_border_width.value() - top_border_width.value() / 6.0,
				  bottom_border_width.value() - bottom_border_width.value() / 6.0);
		cairo_stroke(cr);
	}
}
...
{
	litehtml::pixel_t line_length = std::abs(bottom.value() - top.value());
	if(line_length == 0_px)
	{
		return;
	}
...
void cairo::border::draw_groove_ridge(bool is_groove)
{
	...
	} else
	{
		...
		double line_width = border_width.value() / 2.0;
		cairo_set_line_width(cr, line_width);
		// draw external line
		draw_line(line_width / 2.0, top_border_width.value() / 4.0, bottom_border_width.value() / 4.0);
		set_color(cr, outer_line_color);
		cairo_stroke(cr);
		// draw internal line
		set_color(cr, inner_line_color);
		draw_line(border_width.value() - line_width / 2.0, top_border_width.value() - top_border_width.value() / 4.0,
				  bottom_border_width.value() - bottom_border_width.value() / 4.0);
		cairo_stroke(cr);
	}
}
